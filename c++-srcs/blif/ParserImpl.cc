...
ParserImpl::read_names()
{
  ...
  set_names(oid, names_loc, names_id_list, cover_id);
...
ParserImpl::read_gate()
{
  ...
  for ( ; ; ) {
	...
	return false;
      ...
      set_gate(oid, oloc, id_array, gate_id);
...
ParserImpl::read_latch()
{
  ...
  if ( tk == BlifToken::STRING ) {
    ...
    set_latch(id2, name2_loc, id1, rval);

    mLatchArray.push_back(id2);
...
void
ParserImpl::gen_model(
  BlifModel& model
)
{
  // 入力ノードに番号をつける．
  for ( auto id: mInputArray ) {
    reg_node(id);
  }

  // ラッチノード(の出力)に番号をつける．
  for ( auto id: mLatchArray ) {
    reg_node(id);
  }
  ...
  // に整列される．
  for ( auto id: mOutputArray ) {
    order_node(id);
  }

  // ラッチノードのファンインに番号をつける．
  for ( auto id: mLatchArray ) {
    auto& id_cell = mCellArray[id];
    order_node(id_cell.inode_id());
  }

  model.mName = mModelName;
  model.mNodeList.clear();
  model.mInputNum = 0;
  model.mOutputList.clear();
  model.mLatchNum = 0;
  ...
  for ( auto id: mIdList ) {
    auto& id_cell = mCellArray[id];
    auto name = id2str(id);
    BlifNode* node = nullptr;
    SizeType node_id = model.mNodeList.size();
    if ( id_cell.is_input() ) {
      node = new BlifInput(node_id, name);
      ++ model.mInputNum;
    }
    else if ( id_cell.is_names() ) {
      auto inode_list = make_inode_list(id_cell);
      auto cover_id = id_cell.cover_id();
      node = new BlifNames(node_id, name, inode_list, cover_id);
    }
    else if ( id_cell.is_gate() ) {
      auto inode_list = make_inode_list(id_cell);
      auto cell_id = id_cell.cell_id();
      node = new BlifGate(node_id, name, inode_list, cell_id);
    }
    else if ( id_cell.is_latch() ) {
      auto inode_list = make_inode_list(id_cell);
      char rval = id_cell.rval();
      node = new BlifLatch(node_id, name, inode_list[0], rval);
      ++ model.mLatchNum;
    }
    else {
      ASSERT_NOT_REACHED;
    }
    model.mNodeList.push_back(node);
  }

  for ( auto id: mOutputArray ) {
    ASSERT_COND( mIdMap.count(id) > 0 );
    auto node_id = mIdMap.at(id);
    model.mOutputList.push_back(node_id);
  }
}
...
)
{
  ...
  auto& id_cell = mCellArray[id];
  ASSERT_COND( !id_cell.is_input() );
  for ( auto iid: id_cell.inode_id_list() ) {
    order_node(iid);
  }
  reg_node(id);
}
...
// @brief ファンインのリストを作る．
vector<SizeType>
ParserImpl::make_inode_list(
  const IdCell& id_cell
)
{
  vector<SizeType> inode_list;
  inode_list.reserve(id_cell.inode_size());
  for ( auto id: id_cell.inode_id_list() ) {
    ASSERT_COND( mIdMap.count(id) > 0 );
    auto node_id = mIdMap.at(id);
    inode_list.push_back(node_id);
  }
  return inode_list;
}
